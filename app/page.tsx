...
const LoginScreen = ({ onLogin }: { onLogin: () => void }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if ((u === 'pattuh' && p === 'potato') || (u === 'potato' && p === 'pattuh')) {
      ...
      setError(true); // Show error shake/message
    }
  };

  return (
    <main className="fixed inset-0 z-[9999] flex items-center justify-center p-6 bg-[#FFF5F7] overflow-hidden">
      
      {/* MAGIC CSS INJECTION: 
        This temporarily hides the Navbar and removes the top padding from your layout 
        only while the login screen is active! 
      */}
      ...
      <div className="absolute top-0 left-0 w-[500px] h-[500px] bg-pink-200/40 rounded-full filter blur-[120px] pointer-events-none z-0" />
      <div className="absolute bottom-0 right-0 w-[600px] h-[600px] bg-pink-300/30 rounded-full filter blur-[150px] pointer-events-none z-0" />
      ...
      <motion.div 
        ...
        className="relative z-10 w-full max-w-md bg-white/80 backdrop-blur-md border-2 border-pink-200 rounded-[2rem] p-8 shadow-[0_8px_30px_rgb(255,192,203,0.3)]"
      >
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-serif font-black text-slate-800">Unlock Our Story</h1>
        ...
        <form className="space-y-5" onSubmit={handleLogin}>
          <div>
            ...
            <input 
              ...
              required
              className={`w-full px-4 py-3 bg-pink-50/50 border-2 rounded-xl focus:outline-none focus:bg-white transition-colors text-slate-700 ${
      ...
      */}
      ...
      <div className="relative z-10 text-center max-w-4xl w-full">
        <motion.div 
          ...
          className="flex justify-center items-center gap-4 mb-8 text-pink-500"
        >
          <Gift size={32} />
          <Heart size={28} fill="currentColor" className="animate-pulse drop-shadow-[0_0_15px_rgba(236,72,153,0.5)]" />
          <Gift size={32} />
        </motion.div>

        <div className="relative inline-block mb-6">
          <h1 className="text-5xl md:text-7xl font-serif font-black text-slate-800 leading-tight tracking-tight relative z-10">
        ...
        >
          <p className="text-lg text-slate-600 font-serif max-w-2xl mx-auto mt-6 leading-relaxed bg-white/50 backdrop-blur-sm p-6 rounded-3xl border border-pink-100 shadow-sm relative z-10">
            A messy, beautiful collection of your favorite things, comfort moments, and the little details that make you perfectly 'Us'.
          </p>
        </motion.div>
      </div>

      <div style={{ minHeight: '100px' }} className="w-full relative z-10 flex-shrink-0"></div>
      ...
      <div className="relative z-10 w-full max-w-6xl mx-auto flex flex-col">
        <motion.div 
          ...
          className="bg-white shadow-[0_15px_40px_rgba(244,114,182,0.15)] rounded-[2rem] overflow-hidden border border-pink-100 grid grid-cols-1 md:grid-cols-2 w-full transition-all duration-300"
        >
          <div className="flex flex-col justify-center items-center text-center p-12 md:p-20 bg-white">
            <h2 className="text-3xl md:text-4xl font-serif text-pink-500 font-bold mb-6">Her Carefree Days</h2>
            <p className="text-slate-600 text-lg leading-relaxed font-serif max-w-md">
          ...
          <div className="bg-[#FAE8F0] min-h-[400px] md:min-h-[500px] flex items-center justify-center relative overflow-hidden group">
        ...
        <div style={{ minHeight: '120px' }} className="w-full flex-shrink-0"></div>
        ...
        <motion.div 
          ...
          className="bg-white shadow-[0_15px_40px_rgba(244,114,182,0.15)] rounded-[2rem] overflow-hidden border border-pink-100 grid grid-cols-1 md:grid-cols-2 w-full transition-all duration-300"
        >
          <div className="bg-[#F8E1E8] min-h-[400px] md:min-h-[500px] flex items-center justify-center relative overflow-hidden order-last md:order-first group">
             <motion.div whileHover={{ scale: 1.1, rotate: -5 }} transition={{ type: "spring", bounce: 0.6 }}>
               <img 
          ...
          <div className="flex flex-col justify-center items-center text-center p-12 md:p-20 bg-white">
            <h2 className="text-3xl md:text-4xl font-serif text-pink-500 font-bold mb-6">His Open Roads</h2>
            <p className="text-slate-600 text-lg leading-relaxed font-serif max-w-md">
      ...
      <div style={{ minHeight: '150px' }} className="w-full relative z-10 flex-shrink-0"></div>
      ...
      <motion.div 
        ...
        className="relative z-10 w-full max-w-4xl flex flex-col items-center mb-12"
      >
        <div className="bg-white/80 backdrop-blur-md p-8 md:p-12 rounded-[3rem] shadow-[0_8px_30px_rgb(255,192,203,0.3)] border-2 border-pink-200 w-full text-center">
          
          <div className="relative flex items-center justify-center mb-10 w-full h-12">
            <div className="absolute flex items-center justify-center gap-2">
              <div className="relative flex items-center justify-center">
                 <motion.div 
                    animate={{ scale: [1, 1.8, 1], opacity: [0.6, 0, 0.6] }} 
                    transition={{ repeat: Infinity, duration: 2, ease: "easeInOut" }}
                    className="absolute w-8 h-8 bg-pink-400 rounded-full blur-sm"
                 />
                 <Calendar className="text-pink-500 relative z-10" size={24} />
              </div>
              <h3 className="text-sm font-bold tracking-[0.2em] uppercase text-pink-500 relative z-10 ml-2">
          ...
          <div className="flex flex-wrap gap-4 md:gap-8 justify-center">
            {Object.entries(timeSince).map(([unit, value], i) => (
              ...
              >
                <div className="w-16 h-16 md:w-24 md:h-24 bg-pink-50 rounded-2xl flex items-center justify-center mb-4 shadow-inner border border-pink-100 cursor-default transition-colors hover:bg-pink-100">
                  <span className="font-serif text-4xl md:text-5xl font-black text-slate-800">{value}</span>
                </div>
                <span className="text-xs font-bold tracking-widest text-pink-400 uppercase">{unit}</span>
              </motion.div>
            ))}
          </div>
        </div>
...
export default function App() {
  // Start as 'null' so we don't accidentally flash the login screen before checking memory
  const [isAuthenticated, setIsAuthenticated] = useState<boolean | null>(null);

  useEffect(() => {
    // 1. When the page loads, check if they already unlocked it this session
  ...
  const handleLoginSuccess = () => {
    // 2. When they log in, save that memory into the browser
    sessionStorage.setItem('vaultUnlocked', 'true');
    setIsAuthenticated(true);
  };

  // Wait until we finish checking memory before showing anything
